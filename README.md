...
**3.** Make Geometric Centroid Bounding Box Centroid
Select the object and go to `Scripting` tab from default `Layout` tab in blender. Paste the following code line by line in the left terminal inside `Scripting` tab to execute the task:
```
import bpy
import mathutils
...
```
It will convert the Geometric centroid to the Bounding Box Centroid and the origin of that model will be (0, 0, 0). 
